...
OUTPUT_DIR = ROOT_DIR / '04_echo_chambers' / 'outputs' / 'ml_stance_classification'
...
def train_evaluate_model(train_df, test_df, train_name, test_name):
    ...
    with open(OUTPUT_DIR / f'report_{train_name}_to_{test_name}.txt', 'w') as f:
    ...
    plt.tight_layout()
    try:
        plt.savefig(OUTPUT_DIR / f'cm_{train_name}_to_{test_name}.png', dpi=300)
...
def plot_feature_importance(model, tfidf, le, title_prefix):
    ...
    plt.tight_layout()
    try:
        plt.savefig(OUTPUT_DIR / f'features_{title_prefix.lower()}.png', dpi=300)
