"""
YouTube Data Labeling Script using Gemini API
...
# File paths
INPUT_FILE = 'data/youtube.xlsx'
OUTPUT_FILE = 'data/youtube_labeled.xlsx'
...
def create_prompt_for_batch(batch_df, guidelines):
    """Create a prompt for a batch of YouTube comments"""
    prompt = f"""You are an expert annotator analyzing discourse about the Israel-Hamas war on YouTube.

{guidelines}

Analyze the following {len(batch_df)} YouTube comments and provide labels for each.

For each comment, consider:
- The comment text content
- The author and context
- Any implicit or explicit stance on the conflict
...
"""
    ...
    for idx, row in batch_df.iterrows():
        prompt += f"\n\n--- Item {idx} ---\n"
        prompt += f"Comment ID: {row['id']}\n"
        prompt += f"Video ID: {row['video id']}\n"
        prompt += f"Author: {row['author']}\n"
        prompt += f"Comment Text: {row['text']}\n"
...
def main():
    ...
    if not valid_keys:
        print("ERROR: No API keys found. Please set GEMINI_API_KEY_1, GEMINI_API_KEY_2, GEMINI_API_KEY_3 in .env file")
        return
