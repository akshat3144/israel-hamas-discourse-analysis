"""
Prepare pre-split youtube batch CSV files for faster labeling runs.

This script writes files like:
  batch_0000000000_0000000049.csv

Each batch file includes a __row_index column so label_youtube_data.py can
...
def main():
    parser = argparse.ArgumentParser(description="Pre-split youtube input into batch CSV files")
    parser.add_argument("--input", default="data/youtube.csv", help="Input CSV/XLSX path")
    parser.add_argument("--out-dir", default="data/youtube_batches", help="Output folder for batch CSV files")
